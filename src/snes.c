...
int snes_write_rom(FILE *file,SDL_Surface *image,unsigned char *palette,int npal,int type)
{
	...
	unsigned long *bloc  = malloc(nbloc * sizeof(unsigned long));
    ...
    while(1)
    {
        ...
        if(type == 0) //2,4,8 bpp
        {
            for(y = 0;y <8;y++)
            {
				...
				bloc[ibloc] = binpixel[y] + ((unsigned long)binpixel[y+8]<<16) + ((unsigned long)binpixel[y+16]<<32) + ((unsigned long)binpixel[y+24]<<48);
				ibloc++;
            }
